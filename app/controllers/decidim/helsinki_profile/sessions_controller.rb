    ...
    class SessionsController < ::Decidim::Devise::SessionsController
      ...
      def destroy
        if session.delete("decidim-helsinkiprofile.signed_in")
          ...
          return redirect_to decidim_helsinki_profile.user_helsinki_omniauth_logout_path
        end
      ...
      def end_user_session
        ...
        # HelsinkiProfile to correctly remember where the user needs to be
        # redirected to after a successful sign out.
        id_token_hint = session["omniauth-helsinki.id_token"]
        signed_out = (::Devise.sign_out_all_scopes ? sign_out : sign_out(resource_name))
        store_location_for(:user, redirect_path)
        session["omniauth-helsinki.id_token"] = id_token_hint
