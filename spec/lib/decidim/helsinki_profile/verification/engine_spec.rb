...
describe Decidim::HelsinkiProfile::Verification::Engine do
  it "adds the correct routes" do
    expect(described_class.routes.recognize_path("/authorizations/new")).to eq(
      controller: "decidim/helsinki_profile/verification/authorizations",
      action: "new"
    )
  ...
  it "registers the verification workflow" do
    expect(Decidim::Verifications).to receive(
      :register_workflow
    ).with(:helsinki_idp) do |&block|
      workflow = double
      expect(workflow).to receive(:engine=).with(described_class)
      expect(workflow).to receive(:expires_in=).with(90.days)

      block.call(workflow)
    end

    run_initializer("decidim_helsinki_profile.verification_workflow")
  end

  describe "#load_seed" do
    before { create(:organization) }

    it "adds :helsinki_idp to the organization's available authorizations" do
      described_class.load_seed

      org = Decidim::Organization.first
      expect(org.available_authorizations).to include("helsinki_idp")
    end
  end
